...
export async function getAllGenres() {
  const allGenreEntries = await knex.select().from("Genre");
  const allGenreNames = new Set(
    allGenreEntries.map((entry) => entry.genre_name).flat()
  );
...
export async function getAllCourses() {
  const allCourseEntries = await knex.select("course_name").from("Course");
...
export async function getDirector(name) {
  const director_id = await knex("Directors")
    .select()
    .where({ director_name: name });
